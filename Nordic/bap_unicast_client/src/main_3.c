...
static const bt_addr_le_t target_headset_addr = {
    .type = BT_ADDR_LE_PUBLIC,
    .a.val = {0xFF, 0xE9, 0x5E, 0xDC, 0x99, 0xA8},  // LSB first: reverse of A8:99:DC:5E:E9:FF 
};
...
static const struct bt_bap_lc3_preset template_preset = BT_BAP_LC3_UNICAST_PRESET_16_2_1(
...
static K_SEM_DEFINE(sem_stream_released, 0, 
                         ...
                         struct net_buf_simple *ad)
{
    ...
    if (bt_addr_le_cmp(addr, &target_headset_addr) != 0) {
        return;
    }
 
    char addr_str[BT_ADDR_LE_STR_LEN];
    bt_addr_le_to_str(addr, addr_str, sizeof(addr_str));
    printk("Target headset found: %s (RSSI %d) — connecting\n", addr_str, rssi);
...
static void stream_released(struct bt_bap_stream *stream)
{
    ...
    k_sem_give(&sem_stream_released);
}
				...
				enum bt_security_err err)
{
	...
	k_sem_give(&sem_security_updated); // Always give semaphore
}
...
static int init(void)
{
    ...
    int state_err = app_state_init();
    if (state_err != 0) {
        printk("app_state_init failed: %d\n", state_err);
        return state_err;
    }
...
static int scan_and_connect(void)
{
    ...
    if (err != 0) {
        /* Timed out — stop scanning before returning */
        bt_le_scan_stop();
...
static int configure_streams(void)
{
    ...
    /* Only configure the first sink ASE — sufficient for mono/stereo audio */
...
{
	k_sem_reset(&sem_stream_released);
	k_sem_reset(&sem_connected);
	k_sem_reset(&sem_disconnected);
	k_sem_reset(&sem_mtu_exchanged);
	//k_sem_reset(&sem_security_updated); // No security check
...
int main(void)
{
    codec_configuration = template_preset;
 
    
    codec_configuration.qos.pd = 15000U;       /* .pd represents Presentation Delay */
    codec_configuration.qos.rtn = 2U;          /* Allow up to 2 airtime retransmission bursts */
    codec_configuration.qos.latency = 15U;     /* Set maximum transport latency bounds to 15 ms */
 
    printk("Low-latency configuration applied: Presentation delay balanced to 20ms.\n");
	...
	while (true) {
		...
		/*
		 * LOW-POWER GATE
		 * Block here until the user holds the switch for 2 seconds.
		 * All threads are idle so the SoC enters suspend-to-idle.
		 * Returns with the blue scanning blink already running.
		 */
		app_state_wait_for_start();
 
		printk("Waiting for connection\n");
		err = scan_and_connect();
		if (err == -EAGAIN) {
			/* 10-second timeout — red LED 2 s, then back to low power */
			app_state_notify_scan_timeout();
			continue;
		}
		if (err != 0) {
			return 0;
		...
		/*
		 * Green LED for 2 seconds, then off.
		 * Blocks until the display is done.
		 */
		app_state_notify_connected();
		...
		/* Wait for disconnect — either headset-initiated or long-press */
        ...
        /* Wait for all configured streams to fully release before
         * deleting the group — avoids EINVAL from delete_group() */
        printk("Waiting for streams to release\n");
        for (size_t i = 0; i < configured_stream_count; i++) {
            err = k_sem_take(&sem_stream_released, K_MSEC(2000));
            if (err != 0) {
                printk("Stream %zu release timed out — proceeding\n", i);
            }
        }
        ...
        err = delete_group();
        if (err != 0) {
            printk("delete_group failed: %d — resetting anyway\n", err);
        }
        printk("Group deleted\n");
 
        /*
         * If a long press caused the disconnect, go back to low-power.
         * Otherwise the loop continues and immediately re-enters
         * app_state_wait_for_start() at the top of the next iteration.
         */
        if (app_state_disconnect_requested()) {
            printk("Long press disconnect — returning to low power\n");
        }
        /* Either way, the loop continues to wait_for_start next iteration */
	}
}